                ...
                <tbody>
                    @foreach($transactions as $tx)
                        <tr class="border-b border-line/50 last:border-0 hover:bg-background/50">
                            ...
                            <td class="px-5 py-2.5"><span class="px-2 py-0.5 rounded-md text-xs font-medium {{ $tx->status === 'COMPLETED' ? 'bg-success/10 text-success' : ($tx->status === 'REFUNDED' ? 'bg-warning/15 text-warning' : 'bg-error/10 text-error') }}">{{ $tx->status }}</span></td>
